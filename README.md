...
1.  **Mapeamento BGP**: Consulta ao `RIPE Stat` para identificar todos os prefixos IPv4 anunciados pelo ASN.
...
4.  **Motor de Risco**: Algoritmo de pontuação proprietário que avalia a severidade com base em:
    *   Protocolos expostos (SSH, RDP, Telnet, etc).
    *   Presença de vulnerabilidades conhecidas (CVEs).
    *   Dados de reputação de ameaças (Malware/Spam).
5.  **Relatórios Corporativos**: Geração automática de relatórios técnicos em PDF/Impressão com layout profissional.
